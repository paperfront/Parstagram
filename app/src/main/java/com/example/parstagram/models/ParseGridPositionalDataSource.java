...
public class ParseGridPositionalDataSource extends PositionalDataSource<Post> {


    private User user;
    public ParseGridPositionalDataSource(User user) {
        this.user = user;
    }

    // define basic query here
    public ParseQuery<Post> getQuery() {
        return ParseQuery.getQuery(Post.class).whereEqualTo(Post.KEY_AUTHOR, user).orderByDescending("createdAt").include(Post.KEY_AUTHOR);
    }
